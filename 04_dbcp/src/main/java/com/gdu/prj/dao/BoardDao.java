...
public interface BoardDao {
  ...
  int deleteBoards(String param);  // delete 할 게시물의 번호만 받아오면 됨
  List<BoardDto> selectBoardList(Map<String, Object> map); // 게시글 목록을 몇번부터 몇번까지 받아올지, Map으로 받아온다. 반환타입 : List<> => boardDto가 여러개 들어 있는 list
  int  getBoardCount();                   // 게시글 전체 개수 (SELECT *)
  BoardDto selectBoardByNo(int board_no); // 상세보기 (번호를 받아서 반환), 반환타입 : BoardDto
  void close();                           // BoardDao 가 공동으로 사용 할 자원 반납 메소드 => close() , con.close(), ps.close(), re.close()
}
