...
<%  
  ...
  // 업로드 경로
  String uploadPath = application.getRealPath("upload"); // 학습 외에는 getRealPath 쓰지말 것 => 외부에서 경로 설정해서 써야함
  //                  -----------
  //         서블릿의 request 객체의 getServletContext 메소드와 동일한 객체
  
  File uploadDir = new File(uploadPath);
  if(!uploadDir.exists()){  // 폴더가 없으면 폴더 생성
    uploadDir.mkdirs();
  }
